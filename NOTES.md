...
| libs/mtb.mk | Not tracked in git — regenerated by `make getlibs`. Lists library search paths; do not edit manually |
| libs/*.mtb | Not tracked in git — regenerated by `make getlibs`. URL/commit/path pointers for each library |
